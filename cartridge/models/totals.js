...
function createDiscountObject(collection, discounts) {
    ...
    collections.forEach(collection, function (item) {
        if (!item.basedOnCoupon) {
            totalDiscounts += item.price.value;
            result[item.UUID] = {
                UUID: item.UUID,
...
//     return price;
// }
// function getDiscountPricebook(lineItemContainer){
//     var discountedPrice = 0;
// if(lineItemContainer){
//     var lineItems = lineItemContainer.productLineItems;
...
//                var priceValue = listPrice.value - salesPrice.value;
...
function totals(lineItemContainer) {
    if (lineItemContainer) {
        //this.pricebookDiscountedPrice = getDiscountPricebook(lineItemContainer);
        ...
        this.totalDiscountValue = formatMoney(Money(totalDiscounts,lineItemContainer.currencyCode));
